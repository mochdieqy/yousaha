            ...
            <div>
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0">
                        ...
                        <li class="breadcrumb-item active">Edit {{ $purchaseOrder->number }}</li>
                    </ol>
                </nav>
            </div>
        ...
        <div class="alert alert-info border-0 shadow-sm mb-4">
            <div class="d-flex align-items-center">
                ...
                <div>
                    ...
                    <br>
                    <small class="text-muted">Editing purchase order for this company</small>
                </div>
            </div>
        </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('status') is-invalid @enderror" 
                                ...
                                @if($purchaseOrder->status === 'draft')
                                    ...
                                    <option value="accepted" {{ old('status', $purchaseOrder->status) == 'accepted' ? 'selected' : '' }}>Accepted</option>
                                    <option value="cancel" {{ old('status', $purchaseOrder->status) == 'cancel' ? 'selected' : '' }}>Cancel</option>
                                @else
                                    <option value="draft" {{ old('status', $purchaseOrder->status) == 'draft' ? 'selected' : '' }}>Draft</option>
                                    <option value="accepted" {{ old('status', $purchaseOrder->status) == 'accepted' ? 'selected' : '' }}>Accepted</option>

                                    <option value="done" {{ old('status', $purchaseOrder->status) == 'done' ? 'selected' : '' }}>Done</option>
                                    <option value="cancel" {{ old('status', $purchaseOrder->status) == 'cancel' ? 'selected' : '' }}>Cancel</option>
                                @endif
                            </select>
                            @error('status')
                            ...
                            <div class="card bg-light">
                                <div class="card-body">
                                    <div class="d-flex justify-content-between align-items-center">
                                        ...
                                        <h5 class="mb-0 text-primary" id="totalAmount">Rp {{ number_format($purchaseOrder->total, 0, ',', '.') }}</h5>
                                    </div>
                                </div>
                            </div>
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.product-row').forEach(row => {
        ...
        // Update cost for pre-selected products
        const productSelect = row.querySelector('.product-select');
        const costInput = row.querySelector('.product-cost');
        if (productSelect.value) {
            updateProductCost(productSelect, costInput);
        }
    });
