        ...
        <div class="alert alert-info border-0 shadow-sm mb-4">
            <div class="d-flex align-items-center">
                ...
                <div>
                    ...
                    <br>
                    <small class="text-muted">Sales order will be updated for this company</small>
                </div>
            </div>
        </div>
                ...
                <form action="{{ route('sales-orders.update', $salesOrder) }}" method="POST" id="salesOrderForm">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea class="form-control @error('activities') is-invalid @enderror" 
                                  ...
                                  name="activities" 
                                  rows="3" 
                                  placeholder="Enter activities or notes for this sales order">{{ old('activities', $salesOrder->activities) }}</textarea>
                        @error('activities')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select class="form-select @error('status') is-invalid @enderror" 
                            ...
                            <option value="waiting" {{ old('status', $salesOrder->status) === 'waiting' ? 'selected' : '' }}>Waiting</option>
                            <option value="accepted" {{ old('status', $salesOrder->status) === 'accepted' ? 'selected' : '' }}>Accepted</option>
                            <option value="done" {{ old('status', $salesOrder->status) === 'done' ? 'selected' : '' }}>Done</option>
                            <option value="cancel" {{ old('status', $salesOrder->status) === 'cancel' ? 'selected' : '' }}>Cancel</option>
                        </select>
                        @error('status')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                                ...
                                <div class="product-row row mb-3" data-row="{{ $index }}">
                                        ...
                                        <select class="form-select product-select" name="products[{{ $index }}][product_id]" required>
                                            ...
                                            @foreach($products as $product)
                                                <option value="{{ $product->id }}" 
                                                        ...
                                                        {{ $productLine->product_id == $product->id ? 'selected' : '' }}>
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <input type="text" 
                                               ...
                                               readonly 
                                               value="Rp {{ number_format($productLine->product->price, 0, ',', '.') }}">
                                    </div>
                                    <div class="col-md-1">
                    ...
                    <div class="card border-0 bg-success text-white mb-4">
                        <div class="card-body text-center">
                            <h4 class="mb-0">
                                ...
                                Total: <span id="totalAmount">Rp {{ number_format($salesOrder->total, 0, ',', '.') }}</span>
                            </h4>
                        </div>
                    </div>
...
$(document).ready(function() {
    let productRowCount = {{ count($salesOrder->productLines) }};
    ...
    // Initialize first row remove button visibility
    if (productRowCount > 1) {
        $('.product-row:first .remove-product').show();
    }
});
