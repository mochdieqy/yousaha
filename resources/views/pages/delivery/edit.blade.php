...
<div class="row">
    <div class="col-12">
            ...
            <div>
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0">
                        ...
                        <li class="breadcrumb-item active">Edit #{{ $delivery->id }}</li>
                    </ol>
                </nav>
            </div>
        ...
        <div class="alert alert-info border-0 shadow-sm mb-4">
            <div class="d-flex align-items-center">
                ...
                <div>
                    ...
                    <br>
                    <small class="text-muted">Editing delivery for this company</small>
                </div>
            </div>
        </div>
                ...
                <div>
                    ...
                    <small class="text-muted">
                        @if(in_array($delivery->status, ['draft', 'waiting']))
                            This delivery can be edited. Changes will be applied immediately.
                        @else
                            This delivery cannot be edited in its current status.
                        @endif
                    </small>
                </div>
            </div>
        ...
        <div class="card border-0 shadow-sm">
            ...
            <div class="card-body">
                <form action="{{ route('deliveries.update', $delivery) }}" method="POST" id="deliveryForm" onsubmit="return validateAndSubmit(event)">
                    @csrf
                    @method('PUT')
...
function addProductLine(productId = '', quantity = 1) {
    productLineCount++;
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="product_id_${productLineCount}" name="products[${productLineCount}][product_id]" required>
                                ...
                                @foreach($products as $product)
                                    <option value="{{ $product->id }}" data-sku="{{ $product->sku }}" ${productId == {{ $product->id }} ? 'selected' : ''}>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" id="quantity_${productLineCount}" 
                                   name="products[${productLineCount}][quantity]" min="1" value="${quantity}" required>
                        </div>
                    </div>
                </div>
            </div>
...
// Initialize product lines when page loads
document.addEventListener('DOMContentLoaded', function() {
    // Add existing product lines
    @if($delivery->productLines->count() > 0)
        @foreach($delivery->productLines as $productLine)
            addProductLine({{ $productLine->product_id }}, {{ $productLine->quantity }});
        @endforeach
    @else
        addProductLine();
    @endif
